...
router.put('/addFavPetminder/', Utils.authenticateToken, (req, res) => {  
  ...
  // add petminderId to favouritepetminders field (array - push)
  User.updateOne({
    _id: req.user._id
  }, {
    $push: {
      favouritePetminders: req.body.petminderId
    }
  })
    .then((user) => {            
      res.json({
        message: "Petminder added to favourites"
      })
    })
    .catch(err => {
      console.log(err)
      res.status(500).json({
        message: "Problem adding favourite petminder"
      })
    })
})
...
router.get('/:id', Utils.authenticateToken, (req, res) => {
  ...
  User.findById(req.params.id).populate('favouritePetminder')
    .then(user => {
      res.json(user)
    })
    .catch(err => {
